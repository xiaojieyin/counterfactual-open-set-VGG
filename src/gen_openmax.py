...
def generate(networks, dataloader, **options):
    ...
    gan_scale = options['image_size'] // 16
    ...
    while len(openset_images) < 64:
        ...
        start_images, _ = iter(dataloader).next()
        start_images = Variable(start_images).cuda()
        # end_images, _ = dataloader.get_batch()
        end_images, _ = iter(dataloader).next()
        end_images = Variable(start_images).cuda()
        ...
        preds = F.softmax(netC(images), dim=0)
    ...
    name = 'counterfactual_ge_et_al_{}.jpg'.format(int(time.time()))
...
def make_video_filename(result_dir, dataloader, start_class, target_class, label_type='active'):
    # trajectory_id = '{}_{}'.format('CUB', int(time.time() * 1000))
    trajectory_id = '{}'.format(int(time.time() * 1000))
    # start_class_name = dataloader.lab_conv.labels[start_class]
    # target_class_name = dataloader.lab_conv.labels[target_class]
    start_class_name = str(start_class)
    ...
    video_filename = os.path.join('trajectories_ge_et_al', video_filename)
    video_filename = os.path.join(result_dir, video_filename)
    path = os.path.join(result_dir, 'trajectories_ge_et_al')
