...
def evaluate_classifier(networks, dataloader, open_set_dataloader=None, **options):
    ...
    stats = {
        fold: {
            'classifier_closed_set_accuracy': float(classification_closed_correct) / (classification_total),
        }
    }
    return stats
...
def evaluate_openset(networks, dataloader_on, dataloader_off, **options):
    ...
    auc_d, plot_d = plot_roc(y_true, y_discriminator, 'Discriminator ROC vs {}'.format('CUB'),
                             **options)
...
def openset_weibull(dataloader_test, dataloader_train, netC):
    ...
    for images, labels in dataloader_train:
        images = Variable(images).cuda()
        logits = netC(images)
        correctly_labeled = (logits.data.cpu().max(1)[1] == labels)
