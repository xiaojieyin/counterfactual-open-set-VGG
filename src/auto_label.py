...
def save_image(pixels):
    ...
    filename = os.path.join(options['result_dir'], 'trajectories_{}'.format(options['mode']), uuid.uuid4().hex) + '.png'
    img.save(filename)
...
examples = []
for filename in ls(os.path.join(options['result_dir'], 'trajectories_{}'.format(options['mode'])), '.npy'):
