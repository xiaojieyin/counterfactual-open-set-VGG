...
def train_gan(networks, optimizers, dataloader, epoch=None, **options):
    ...
    for i, (images, class_labels) in enumerate(dataloader):
        ...
        labels = torch.ones(options['batch_size'], options['num_classes']) * -1
        labels = labels.scatter_(1, torch.unsqueeze(class_labels, dim=1), 1).cuda()  # one-hot

        # ac_scale = random.choice([1, 2, 4, 8])
        # ac_scale = 4
        # sample_scale = 4
        ac_scale = options['image_size'] // 16
        sample_scale = options['image_size'] // 16
        ############################
    ...
    results = {
        'errC': errC.item(),
        'errG': errG.item(),
        'err_reconstruction': err_reconstruction.item(),
    }
    return results
